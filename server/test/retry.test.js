...
test('transient failure → backoff → reclaim → heals to published', async () => {
  ...
  await db.insert(schema.socialPosts).values({ id: 'sp_t', workspaceId: 'ws_r', groupId: 'g1', accountId: 'sa_m', platform: 'mastodon', body: 'hi', status: 'scheduled', scheduledAt: past });
...
test('permanent failure (char limit) never queues a retry', async () => {
  const past = new Date(Date.now() - 60_000);
  await db.insert(schema.socialPosts).values({ id: 'sp_p', workspaceId: 'ws_r', groupId: 'g2', accountId: 'sa_b', platform: 'bluesky', body: 'x'.repeat(301), status: 'scheduled', scheduledAt: past });
